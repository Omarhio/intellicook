<script lang="ts">
	...
	function handlePrevious() {
		...
		dispatch('previous', { index: currentIndex });
		startAutoSlide();
	}
	...
	{#if items.length > 0}
				...
				>
					...
					>
						<img
							...
							on:error={() => handleImageError(previousIndex)}
						/>
				...
				>
					...
					>
						<img
							...
							on:error={() => handleImageError(currentIndex)}
						/>
						<div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
				...
				>
					...
					>
						<img
							...
							alt=""
							class="h-full w-full transform object-cover transition-transform duration-300 hover:scale-110"
							on:error={() => handleImageError(nextIndex)}
						/>
						<div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
		...
		<div class="mt-4 flex justify-center space-x-2" role="tablist">
			{#each items as _, i}
				<button
					type="button"
					class="h-2 w-2 rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-[#F4ACB7] {i === currentIndex ? 'bg-[#F4ACB7]' : 'bg-[#F4ACB7]/30 hover:bg-[#F4ACB7]/50'}"
